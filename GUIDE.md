...
| 태스크 | env · userenv · cert · certstore · wininet · node · npm · claude · codex · wt · browser (11종) |
| 브라우저 대응 | 인증서 신뢰 저장소 등록(certstore) · WinINET/WinHTTP 프록시(wininet) · Edge·Chrome 접속 진단(browser) |
...
src-tauri/src/                    ── Rust 백엔드
  ...
  detect.rs                       네트워크 감지
  win/registry.rs                 HKLM/HKCU 레지스트리 읽기/쓰기/삭제 · WinINET 프록시 · 정책 조회
  win/broadcast.rs                WM_SETTINGCHANGE 브로드캐스트
  win/wininet.rs                  InternetSetOption — WinINET 설정 변경 통지
...
// [browser] 진단 태스크가 프록시 경유 접속을 점검할 (이름, URL) 목록
...
- **Edge/Chrome 인증서 경고가 계속됨**: [certstore] 완료 후에도 경고가 남으면 브라우저를
  **완전 종료**(작업 관리자에서 msedge/chrome 프로세스 확인) 후 재시작하세요. 특정 사이트만
  안 되면 `edge://net-internals/#hsts` 에서 해당 도메인 HSTS 삭제 + 캐시 삭제(Ctrl+Shift+Del).
- **브라우저만 인터넷이 안 됨**: [browser] 진단을 실행해 WinINET 프록시 적용 여부와
  GPO 정책 강제 여부를 확인하세요. 정책(HKLM\Policies\Microsoft\Edge 등)이 있으면
  인터넷 옵션보다 우선하므로 IT 관리 부서에 문의해야 합니다.
```
